...
class SearchAgents(AgentFactory):
...
class OffensiveReflexAgent(CaptureAgent):
    def registerInitialState(self, gameState):
        ...
        for x, y in self.legalPositions:
            ...
            self.searchFlag = BFS(start_pos, remain_food)
        

    def BFS(pos, remain_food):
        if len(remain_food) > 2:
            return True
        for new_pos in self.legalPositions():
